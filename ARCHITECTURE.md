...
3. **KLANGK_LLM_BASE_URL doesn't need to change**: The Pi configuration uses `${KLANGK_LLM_BASE_URL}` directly, so no per-container configuration is needed — nginx handles the routing.
...
- Orange Klangk logo (paw icon + "Klangk" text)
