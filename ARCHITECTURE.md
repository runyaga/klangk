...
nginx reverse proxy (port 8995, serves UI + API + hosted app proxy + LLM proxy)
    ↕ LLM proxy: container → host.docker.internal:8995/llm-proxy/ → ${BARK_LLM_BASE_URL}
    ↕
...
Pi containers access the LLM via the **LLM proxy**, an nginx location that proxies `/llm-proxy/` requests to `${BARK_LLM_BASE_URL}`. This is required because:
...
3. **BARK_LLM_BASE_URL doesn't need to change**: The Pi configuration uses `${BARK_LLM_BASE_URL}` directly, so no per-container configuration is needed — nginx handles the routing.
...
- **LLM Provider**: Any OpenAI-compatible LLM provider (Ollama Cloud, self-hosted Ollama, etc.), configurable via env vars (`BARK_LLM_BASE_URL`, `BARK_LLM_MODEL`, `BARK_LLM_API_KEY`). The model must support tool/function calling — Pi uses tools (bash, edit, write, read) to interact with the workspace.
...
bark/
  ...
  .env                          # Secrets: BARK_LLM_API_KEY, BARK_LLM_BASE_URL, BARK_LLM_MODEL, BARK_JWT_SECRET, etc.
  .gitignore
  README.md
...
- All provider env vars (`BARK_LLM_*`, `ANTHROPIC_*`, etc.) stripped from Pi's process environment before exec
...
| `BARK_IMAGE_NAME`             | `bark`                               | Docker image name for workspace containers                                                                                                                |
