...
- **Delete stale skipped CI runs**: The hourly E2E schedule uses a two-job pattern (check + test) so no-op runs show as "skipped" rather than misleading green "success". But skipped runs still accumulate in the Actions UI. Consider a periodic cleanup action or GitHub API script to delete old skipped runs.
