...
Today Klangk is consumed exclusively through the Flutter web UI. You want to use Klangk's container isolation for your normal development workflow — dropping into a shell inside a Klangk-managed container to work on projects like Klangk itself. The CLI should work both locally and against a remote Klangk server (e.g., from a coffee shop connecting to arctor).
...
- A future `klangk pi` CLI command could auto-launch Pi in the shell (like the web UI's dedicated Pi pane) but isn't needed for phase 1

## Approach

A `klangk` CLI that lives inside the existing backend package as a thin client. It talks to the running Klangk backend over HTTP + WebSocket — same infrastructure the Flutter UI uses. The shell session uses the backend's existing `terminal_manager.py` (WebSocket → `docker exec` PTY).
...
```text
src/backend/klangk_backend/cli/
    __init__.py
    main.py          # typer app, command group
    auth.py          # login/logout, token storage
    client.py        # HTTP + WebSocket client wrapper
    config.py        # CLI config (~/.config/klangk/cli.toml)
```

Add to `pyproject.toml`:

- Dependencies: `typer[all]>=0.12.0` (brings rich, click)
- Entry point: `klangk = "klangk_backend.cli.main:app"`

## Commands

| Command                                                             | Description                                                                      |
| ------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `klangk login [EMAIL] [--server URL] [--password-file FILE]`        | Authenticate. Reuses saved token if valid. `--password-file -` reads from stdin. |
| `klangk logout`                                                     | Clear stored token                                                               |
| `klangk status [--plain]`                                           | Show connection info (server, user, login status)                                |
| `klangk list [--plain]`                                             | List workspaces                                                                  |
| `klangk create NAME [--mount SRC:DST] [--env KEY=VALUE]`            | Create a workspace (with optional mounts and env vars)                           |
| `klangk edit NAME [--name] [--image] [--command] [--mount] [--env]` | Edit workspace settings (flags mode or interactive)                              |
| `klangk dup SOURCE NEW_NAME`                                        | Duplicate a workspace (copies image, command, mounts, env)                       |
| `klangk rm NAME`                                                    | Delete a workspace                                                               |
| `klangk shell [WORKSPACE]`                                          | **Main command.** Connect to workspace, drop into bash inside the container.     |
| `klangk exec WORKSPACE COMMAND...`                                  | Run a command in a container. Also usable as an rsync transport.                 |
| `klangk sync SRC DEST`                                              | Sync files to/from a container via rsync (wraps `klangk exec`).                  |
| `klangk volumes ls [--plain]`                                       | List klangk-managed Docker volumes                                               |
| `klangk volumes create NAME`                                        | Create a named Docker volume                                                     |
| `klangk volumes rm NAME`                                            | Delete a named Docker volume                                                     |

## `klangk shell` — The Core Flow
...
`klangk shell` must be a well-behaved terminal program that works inside tmux, herdr, cmux, and similar orchestrators. This means:
...
- `klangk login [EMAIL]`, `klangk logout`, `klangk status [--plain]`
- `klangk list [--plain]`, `klangk create`, `klangk rm` (HTTP client via httpx)
- `klangk shell` — WebSocket terminal with raw mode, stdin/stdout forwarding, select-based interruptible stdin, SIGWINCH via polling
- `--password-file` for non-interactive login (scripting)
- Token reuse: `klangk login` verifies saved token before prompting
...
- `--mount PATH` flag on `klangk shell`
...
- `klangk ssh` command or direct `ssh` with key management
...
- Currently hardcoded to `klangk` image via `IMAGE_NAME` in `container.py`
...
- CLI impact: add `--image` flag to `klangk create`, no other commands change
...
- `klangk exec WORKSPACE COMMAND...` — raw command execution in containers via WebSocket (`ExecSession` with piped stdin/stdout, no PTY)
- `klangk sync SRC DEST` — rsync wrapper using `klangk exec` as the transport
...
| File                                             | Purpose                                        |
| ------------------------------------------------ | ---------------------------------------------- |
| `src/backend/klangk_backend/cli/main.py`         | Typer app, top-level + `ws` subcommand group   |
| `src/backend/klangk_backend/cli/auth.py`         | Login/logout with token reuse and rich prompts |
| `src/backend/klangk_backend/cli/client.py`       | HTTP + WebSocket client, shell/exec forwarding |
| `src/backend/klangk_backend/cli/config.py`       | Config storage (~/.config/klangk/cli.toml)     |
| `src/backend/klangk_backend/exec_session.py`     | Backend raw exec session (no PTY)              |
| `src/backend/klangk_backend/terminal_manager.py` | Backend PTY session (docker exec)              |
| `src/backend/klangk_backend/ws_handler.py`       | WebSocket terminal + exec protocol             |
