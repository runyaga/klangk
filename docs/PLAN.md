...
- **Chat input box stays expanded after submit**: The chat input field auto-expands upward when the user types a long prompt that overflows, which is correct. But after submitting the prompt, the input box stays at the expanded height instead of resetting to its default single-line size.
