...
- **E2E tests** (`.github/workflows/e2e-tests.yml`) — runs hourly via schedule (skips if no commits in the last hour) and via manual `workflow_dispatch`. Requires `OLLAMA_API_KEY`, `OLLAMA_BASE_URL`, and `OLLAMA_MODEL` secrets. Runs Playwright against Chromium, Firefox, and WebKit sequentially (browsers from `playwright-driver.browsers` in nixpkgs, one worker per CPU core). Uploads test results as artifacts on failure.
