...
- **LLM proxy to avoid passing credentials to containers**: Run a localhost proxy on the host that forwards LLM API requests (Ollama, OpenAI, Anthropic, etc.) and injects the API key on the host side. Containers would hit `http://host.docker.internal:<port>/v1/...` with no credentials — the proxy adds the `Authorization` header before forwarding to the real LLM endpoint. This eliminates passing `OLLAMA_API_KEY` and other provider secrets into containers entirely. The FIFO mechanism in `entrypoint.sh` (which exists to prevent the bark user from reading `models.json` after Pi startup) would no longer be needed. Could use nginx, Caddy, or a simple Python/Go reverse proxy. Also solves the OTEL `/proc` secret visibility issue.
