...
- **Add --release to flutterbuildweb**: The `flutterbuildweb.sh` script doesn't pass `--release` to `flutter build web`. Add it for smaller, faster builds.
...
- **Simplify Docker build execIfModified paths**: The `bark:docker-build` task in `devenv.nix` lists individual files in `execIfModified` (`Dockerfile`, `entrypoint.sh`, `bash.bashrc`, `*.md`, etc.). Use a single glob `src/dockerimage/*` instead so new files are automatically tracked without needing to update `devenv.nix`.
