...
- **Strip OTEL env vars from terminal session**: `OTEL_EXPORTER_OTLP_HEADERS` contains the Logfire write token and is visible via `env` in the container terminal. Can't strip it via `docker exec env -u` because the pi-otel-telemetry extension needs it. Options: have the entrypoint write OTEL vars to a file that the extension reads instead of env vars, or configure the extension to read the token from a file directly.
